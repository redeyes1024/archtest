...
LOG="Install-Logs/01-i3wm-Install-Scripts-$(date +%d-%H%M%S).log"
...
whiptail --title "Arch i3wm (2026) Install Script" \
    --msgbox "Welcome to Arch i3wm (2026) Install Script!\n\n\
ATTENTION: Run a full system update and reboot first (highly recommended).\n\n\
NOTE: If you are installing on a VM, enable 3D acceleration for better Xorg performance." \
    15 80
...
if ! groups "$(whoami)" | grep -q '\binput\b'; then
    ...
    whiptail --title "Input Group" --msgbox "You are not currently in the input group.\n\nAdding you to the input group can be useful for some input/keyboard integrations, depending on your setup." 12 60
fi
...
options_command+=(
    ...
    "xdph" "Install XDG Desktop Portal (for screen share)?" "OFF"
    "zsh" "Install zsh shell with Oh-My-Zsh?" "OFF"
    "pokemon" "Add Pokemon color scripts to your terminal?" "OFF"
    "rog" "Are you installing on Asus ROG laptops?" "OFF"
    "dots" "Install pre-configured i3wm dotfiles from this repo?" "OFF"
)
...
while true; do
    ...
    if [[ "$dots_selected" == "OFF" ]]; then
        ...
        if ! whiptail --title "i3wm Dotfiles" --yesno \
        "You have not selected to install the pre-configured i3wm dotfiles.\n\nIf you proceed without Dots, i3 will start with its default configuration.\n\nWould you like to continue without installing dotfiles or return to choices/options?" \
    ...
    echo "👌 ${OK} You confirmed your choices. Proceeding with ${SKY_BLUE}i3wm installation...${RESET}" | tee -a "$LOG"
    break  
done
...
# Run the i3 related scripts
echo "${INFO} Installing ${SKY_BLUE}i3wm additional packages...${RESET}" | tee -a "$LOG"
sleep 1
execute_script "01-i3-pkgs.sh"
...
echo "${INFO} Installing ${SKY_BLUE}i3wm + Xorg...${RESET}"
sleep 1
...
for option in "${options[@]}"; do
    case "$option" in
        ...
        input_group)
            ...
            ;;
        xdph)
            echo "${INFO} Installing ${SKY_BLUE}xdg-desktop-portal (screen share support)...${RESET}" | tee -a "$LOG"
        ...
        rog)
            ...
            ;;
        dots)
            echo "${INFO} Installing pre-configured ${SKY_BLUE}i3wm dotfiles...${RESET}" | tee -a "$LOG"
...
if pacman -Q i3-wm &> /dev/null; then
    printf "\n ${OK} 👌 i3wm is installed. However, some essential packages may not be installed. Please see above!"
    printf "\n${CAT} Ignore this message if it states ${YELLOW}All essential packages${RESET} are installed as per above\n"
    sleep 2
    printf "\n%.0s" {1..2}

    printf "${SKY_BLUE}Thank you${RESET} for using this installer. ${YELLOW}Enjoy and have a good day!${RESET}"
    printf "\n%.0s" {1..2}

    printf "\n${NOTE} If you did not install SDDM, you can start i3 via ${SKY_BLUE}startx${RESET} (ensure you have an ${YELLOW}~/.xinitrc${RESET} that launches i3).\n"
    ...
    printf "\n${WARN} i3wm is NOT installed. Please check the logs in the Install-Logs/ directory..."
    printf "\n%.0s" {1..3}
    exit 1
fi
